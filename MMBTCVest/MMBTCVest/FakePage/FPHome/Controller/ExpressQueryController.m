...
#import "ExpressQueryController.h"

#import "ExpressQueryModels.h"
#import "ExpressQueryModel.h"


@interface ExpressQueryController ()
...
@implementation ExpressQueryController
...
- (void)preparUI {
    ...
    queryTextField.placeholder = @"输入快递单号查询";
    ...
    //    verificationTitleLabel.text = @"快递号查询系统，请勿用于非法用途!";
...
- (void)submitClick {
    ...
    NSDictionary *parameters = @{@"showapi_appid":@"43838",@"showapi_sign":@"2b8b3da87db542f4900e4102f93dba6a", @"com": @"auto", @"nu": self.queryTextField.text};
    
    [self showLoadingView];
    
    NSString *requestUrl = @"http://route.showapi.com/64-19";
  ...
  [BANetManager ba_request_POSTWithEntity:entity successBlock:^(id response) {
    ...
    ExpressQueryModels *models = [ExpressQueryModels mj_objectWithKeyValues:response];
    ...
    if (models.showapi_res_code == 0) {
      ...
      ExpressQueryModel *model = models.showapi_res_body;
      
      if (model.ret_code == 0) {
        self.queryResultLabel.text = [NSString stringWithFormat:@"快递公司名称: %@\n\n公司英文: %@\n\n快递单号: %@\n\n联系电话: %@\n\n快递配送信息: %@\n\n快递名: %@\n\n", model.expTextName, model.expSpellName, model.mailNo,model.tel,model.data, model.expName];
      } else {
        [self showHint:@"查询失败"];
      }
